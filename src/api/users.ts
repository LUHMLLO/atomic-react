...
export async function GetUser(id: string) {
    ...
    try {
        const response = await axios.get(`${import.meta.env.VITE_ENDPOINT_USERS_READ}${id}`, {
            headers: {
                Authorization: `Bearer ${jwt}`,
                'Content-Type': 'application/json',
            },
...
export async function CreateUser(data: UserDTO, navigate?: (path: string) => void) {
    const jwt = $token.get();

    try {
        const response = await axios.post(import.meta.env.VITE_ENDPOINT_USERS_HANDLE, data, {
            headers: {
                Authorization: `Bearer ${jwt}`,
                'Content-Type': 'application/json',
            },
        ...
        if (response.status === 200) {
            $setNotification({ title: 'Success', content: 'new user has been created', icon: 'check_circle' })
            if (navigate) {
                navigate('/users');
            }
        } else {
            $setNotification({ title: 'Error', content: 'there has been an error creating user', icon: 'error' })
        }
    } catch (error: unknown) {
        HandleApiError(error)
    }
